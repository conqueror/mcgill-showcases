...
| RL for agentic tutoring: learned intervention policy around a deterministic assistant | `projects/adaptive-course-assistant-rl-showcase` | `make run` | `artifacts/assistant/episode_trace.json`, `artifacts/eval/offline_policy_eval.csv`, `artifacts/bridge/policy_router.json`, `artifacts/bridge/learning_agent_story.md`, `artifacts/business/deployment_recommendation.md` |
| Learning an agent's orchestration policy without retraining LLM weights | `projects/learning-agents-showcase` | `make smoke` | `artifacts/concepts/algorithm_progression.md`, `artifacts/eval/policy_comparison.csv`, `artifacts/business/deploy_shadow_reject_memo.md` |
