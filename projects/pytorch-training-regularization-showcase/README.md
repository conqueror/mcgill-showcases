...
The files under `artifacts/` are generated by `make smoke`, `make run`, or the focused
experiment commands and are intentionally not committed. In a clean checkout,
`make verify` still knows the stable required artifact contract, even if
`artifacts/manifest.json` has not been generated yet.
...
If verification lists missing files, rerun `make smoke` or `make run` and then inspect
the generated metrics, training history, and error-analysis artifacts.
