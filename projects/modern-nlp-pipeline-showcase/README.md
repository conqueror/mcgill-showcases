...
The clean-checkout dataset lives in
`src/modern_nlp_pipeline_showcase/sample_data/` so tests, `make smoke`, and CI do not
depend on ignored `data/raw/` files. To teach with a custom corpus, add local override
files at:

- `data/raw/research_corpus.csv`
- `data/raw/research_queries.json`

Those raw overrides are intentionally local-only and should not be committed.

The files under `artifacts/` are generated by `make smoke` or `make run` and are also
local outputs. Run `make verify` after generating artifacts to confirm the expected
classification, retrieval, generation, summary, and manifest files are present.
...
- Tests fail because custom raw files are malformed:
  Remove the local files under `data/raw/` to fall back to the bundled sample corpus,
