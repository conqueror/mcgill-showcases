"""Tabular off-policy temporal-difference control via Q-learning.

Implements one-step Q-learning, the off-policy companion to SARSA: it behaves with an
exploratory epsilon-greedy policy but bootstraps its backup from ``max_a' Q(s', a')`` --
the value of a *greedy* next action it may not actually take. That decoupling of the
behaviour policy from the target is exactly what makes Q-learning off-policy and lets it
chase the Bellman optimality action values Q*(s,a) regardless of how exploratory the data
generation was. On the algorithm ladder
(contextual bandit -> MDP -> Q-learning -> DQN -> policy gradient -> actor-critic -> PPO)
this is the value-based rung that DQN later scales to function approximation.

RL concept:
    Off-policy TD control / Q-learning; see docs/value-based-learning.md and
...
class QLearningResult:
    ...
    What + why: bundles the estimate of Q*(s,a) with a diagnostic learning curve so a
    Q-learning run is reproducible and directly comparable to its on-policy twin
    ``SarsaResult`` (identical shape) in notebooks and artifact pipelines. RL concept: it
    sits on the value-based rung of the ladder
    (contextual bandit -> MDP -> Q-learning -> DQN -> policy gradient -> actor-critic -> PPO),
    one step beside on-policy SARSA.
    ...
    def greedy_policy(self) -> QLearningPolicy:
        ...
        What + why: training explores with epsilon-greedy, but because Q-learning is
        off-policy its target already estimates the *greedy* value Q*(s,a), so deployment
        simply reads off that greedy action. RL concept: greedy action a* = argmax_a Q(s,a)
        recovers the optimal policy implied by the learned value function.
...
def train_q_learning(
    *,
    ...
    What + why: the agent explores with an epsilon-greedy *behaviour* policy but always
    backs up toward ``max_a' Q(s', a')`` -- the value of the *greedy* next action rather
    than the one it actually takes. That max is the entire off-policy mechanism: it lets the
    update target the optimal policy while the data is generated by an exploratory one, so
    the table converges toward the Bellman optimality action values Q*(s,a). Its only
    structural difference from SARSA is this backup target. RL concept: off-policy TD control
    on the value-based rung of the ladder
    (contextual bandit -> MDP -> Q-learning -> DQN -> policy gradient -> actor-critic -> PPO);
    DQN later replaces the table with a neural function approximator.
    ...
    Returns:
        A ``QLearningResult`` with the learned Q-table and the per-episode training curve.
...
def q_table_rows(q_table: dict[StateKey, list[float]]) -> list[dict[str, int | float]]:
    ...
    What + why: unpacks each six-int state key into named columns and emits one row per
    action with its value Q(s,a), giving a long-format table for export, plotting, or
    inspecting which intervention the policy prefers in each state. State keys are sorted so
    the output is deterministic and diff-friendly. RL concept: a direct read-out of the
    learned action-value function Q(s,a); no learning happens here.

    Args:
        q_table: Action values Q(s,a) keyed by the six-int state tuple.

    Returns:
        Rows with columns
        ``week, engagement, completion, pressure, risk, prior_interventions, action,
        q_value`` (one row per state-action pair, q_value rounded to 6 dp).
...
def _epsilon_greedy_action(
    ...
    What + why: this is Q-learning's *behaviour* policy -- the exploratory rule that
    generates training data. Because Q-learning is off-policy, this exploration is decoupled
    from the greedy *target* the update chases, so wide exploration here does not bias the
    estimate of Q*(s,a). RL concept: epsilon-greedy balances exploration vs exploitation over
    action values Q(s,a).
